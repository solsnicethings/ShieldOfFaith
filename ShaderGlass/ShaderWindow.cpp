...
void ShaderWindow::CompileThreadFunc()
{
    ...
    while(true)
    {
        ...
        try
        {
            ...
            PostMessage(m_mainWindow, WM_COMMAND, WM_SHADER(id), 0);
        }
        catch(file_error& ex)
        {
...
void ShaderWindow::UpdateTitle()
{
    ...
    if(m_captureManager.IsActive())
    {
        ...
        char outFPSdisplay[20] = "";
        if(m_captureManager.ActiveSubFrames())
            snprintf(outFPSdisplay, 20, "%d/%dfps", outFPS, m_captureManager.ActiveSubFrames());
        else if(m_captureManager.Synchronous())
            snprintf(outFPSdisplay, 20, "%d/Sfps", outFPS);
        else
            snprintf(outFPSdisplay, 20, "%dfps", outFPS);
        _snwprintf_s(title,
                     200,
                     _T("ShaderGlass (%s%S, %Spx, %S%%, ~%S, %S%S%S)"),
                     windowName,
                     shader->Name.c_str(),
                     pixelSize.mnemonic,
                     scaleString,
        ...
        default:
            if(wmId >= WM_USER && wmId <= 0x7FFF)
            {
                if(wmId >= WM_SHADER(0) && wmId < WM_SHADER(MAX_SHADERS))
                {
                    ...
                    auto newPresetNo = wmId - WM_SHADER(0);
                    if(m_captureManager.RestartRequired(newPresetNo))
                    {
                        m_captureManager.StopSession();
                        m_captureOptions.presetNo = newPresetNo;
...
void ShaderWindow::ToggleBorderless(HWND hWnd)
{
    ...
    ShaderWindow::ipc_update_layout();
}
...
bool ShaderWindow::Create(_In_ HINSTANCE hInstance, _In_ int nCmdShow)
{
    ...
    // default subframes
    {
        ...
        if(EnumDisplaySettings(NULL, ENUM_CURRENT_SETTINGS, &lpDevMode) != 0)
        {
            ...
            {
                int subframes = 0;
                for(int sf = 2; sf <= 9; sf++)
                {
                    // 60 Hz base
                    if(abs(sf * 60 - refreshRate) <= 2)
                    {
                        subframes = sf;
                        break;
                    }
                    // 50 Hz base
                    if(abs(sf * 50 - refreshRate) <= 2)
                    {
                        subframes = sf;
                        break;
                    }
...
static unsigned int redefine_depth = 0;

struct RemoteShaderParam
{
public:
    int   distinct_values, current_relative_value;
    float relative_value_multiplier;

    RemoteShaderParam(ShaderParam* param)
    {
        ShaderParam* info       = param;
        auto         range_size = info->maxValue - info->minValue;
        ...
        relative_value_multiplier = info->stepValue;
        if(!relative_value_multiplier)
        {
            relative_value_multiplier = range_size / 100.0f;
            distinct_values           = 100;
        }
        else
            distinct_values = static_cast<int>(range_size / relative_value_multiplier);
        current_relative_value = (info->currentValue - info->minValue) / relative_value_multiplier;
    }
};
...
/*
static bool window_restore_thread_running = false;

static void window_restore_thread_func(HWND window) {
    ShowWindowAsync(window, SW_SHOWNOACTIVATE);
    window_restore_thread_running  = false;
}*/

#define ipcwinstate_request_fs 0x1
#define ipcwinstate_request_win 0x2
#define ipcwinstate_request_min 0x4
#define ipcwinstate_request_re 0x8
...
void ShaderWindow::ipc_update_layout_callback(IPCWindow* ipc)
{
    ...
    //ipc->RequestShowWindowCall(shader->m_mainWindow, SW_SHOWMINNOACTIVE);
    //ipc->RequestShowWindowCall(shader->m_mainWindow, SW_SHOWNOACTIVATE);
...
static void ipcwin_OnDefinitionRequest(IPCWindow* ipc) {
    ((ShaderMessageWindow*)ipc)->OnDefinitionRequest((ShaderMessageWindow*)ipc);
}

void ShaderWindow::ipc_new_profile(IPCWindow* ipc)
{
    if(ipc->ExtendedGetState() < IPCWCode::HandshakeComplete)
        return;
    auto shader = (ShaderWindow*)ipc->CustomRef;
...
void ShaderWindow::ipc_new_shader(IPCWindow* ipc)
{
    if(ipc->ExtendedGetState() < IPCWCode::HandshakeComplete)
        return;
    ...
    if(shader->m_captureOptions.presetNo < shader->m_captureManager.Presets().size())
    {
        ...
        std::wstring path;
        if (cfg->Category == "Imported")
        {
            path = cfg->ImportPath.wstring();
        }
        else
        {
            path = L"\"";
            auto s = convertCharArrayToLPCWSTR(cfg->Name.c_str());
            path.append(s);
            delete[] s;
            path.append(L"\" from ");
            s = convertCharArrayToLPCWSTR(cfg->Category.c_str());
            path.append(s);
            delete[] s;
        }
...
void ShaderWindow::ipc_init()
{
    if(m_ipcWindow.ExtendedGetState())
        return;
    ...
    m_ipcWindow.OnDefinitionRequest = [](ShaderMessageWindow* ipc) {
        ...
        for(auto p : shader->m_captureManager.Params())
        {
            //if(d != redefine_depth)
              //  goto endredefine;
            auto rp = RemoteShaderParam(std::get<1>(p));
            if(!rp.distinct_values)
                continue;
        ...
        ipc->RunOnce(ipc_new_profile);
        ipc->RunOnce(ipc_new_shader);
    ...
    m_ipcWindow.OnGetParameterRelativeValue = [](int relative_value, unsigned int id, ShaderMessageWindow* ipc) {
        ...
        auto shader = (ShaderWindow*)ipc->CustomRef;
        auto v      = shader->m_captureManager.Params();
        if(--id >= v.size())
            return;
        auto p          = std::get<1>(v[id]);
        auto r          = RemoteShaderParam(p);
        p->currentValue = p->minValue + r.relative_value_multiplier * relative_value;
        redefine_depth  = 1;
        try
        {
            shader->m_captureManager.UpdateParams();
        }
        catch(std::exception ex)
        {
            redefine_depth = 0;
    ...
    m_ipcWindow.ResolveNameToNonZeroId = [](LPCWSTR name, IPCWindow* ipc) {
        auto         shader = (ShaderWindow*)ipc->CustomRef;
        unsigned int i      = 1;
        for(auto p : shader->m_captureManager.Params())
        {
            auto n = convertCharArrayToLPCWSTR(std::get<1>(p)->name.c_str());
            if(wcscmp(name, n) == 0)
            {
                delete[] n;
                return i;
    ...
    m_ipcWindow.OnSetFullscreenCommand = [](bool fs, ShaderMessageWindow* ipc) {
        ipc_SendDiagnosticMessage(ipc, fs ? L"OnSetFullscreenCommand(ON)" : L"OnSetFullscreenCommand(OFF)");
        auto shader = (ShaderWindow*)ipc->CustomRef;
        if(fs)
    ...
    m_ipcWindow.OnSetMinimizedCommand = [](bool min, ShaderMessageWindow* ipc) {
        ipc_SendDiagnosticMessage(ipc, min ? L"OnSetMinimizedCommand(ON)" : L"OnSetMinimizedCommand(OFF)");
        auto shader = (ShaderWindow*)ipc->CustomRef;
        if(min)
    ...
    m_ipcWindow.OnToggleMenuCommand = [](ShaderMessageWindow* ipc) {
        ...
        if(GetMenu(hWnd))
            ...
            SetMenu(hWnd, shader->m_mainMenu);
    };

    /*
    m_ipcWindow.OnPartnerShuttingDown = [](IPCWindow* ipc) { 
        return (LRESULT)DestroyWindow(((ShaderWindow*)ipc->CustomRef)->m_mainWindow); 
    };
    */

    m_ipcWindow.RunAfterCreated([](IPCWindow* win) { win->BeginShakingHandsUntilSuccessful(); });

    m_captureManager.ipc_after_redefine  = [](void* me) { ((ShaderWindow*)me)->ipc_after_redefine(); };
    m_captureManager.ipc_before_redefine = [](void* me) { ((ShaderWindow*)me)->ipc_before_redefine(); };
    m_captureManager.ipc_update_params   = [](void* me) { ((ShaderWindow*)me)->ipc_update_params(); };
    m_captureManager.ipc_loaded_shader   = [](void* me) { ((ShaderWindow*)me)->ipc_new_shader(&((ShaderWindow*)me)->m_ipcWindow); };
    m_captureManager.ipc_arg             = this;

    m_ipcWindow.Create(GetModuleHandle(NULL));
}

void ShaderWindow::ipc_before_redefine()
{
    if(m_ipcWindow.ExtendedGetState() >= IPCWCode::HandshakeComplete)
    {
        ++redefine_depth;
...
void ShaderWindow::ipc_after_redefine()
{
    if(m_ipcWindow.ExtendedGetState() >= IPCWCode::HandshakeComplete)
    {
...
void ShaderWindow::ipc_update_params()
{
    ...
    for(auto p : m_captureManager.Params())
    {
        ...
        {
            auto name = convertCharArrayToLPCWSTR(std::get<1>(p)->name.c_str());
            m_ipcWindow.SendParameter(0, rp.current_relative_value, name, nullptr);
            delete[] name;
        }
    }
}
